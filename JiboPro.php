...
function JibuPro_add_menu_links() {
	...
	add_submenu_page('quiz-plug', 'Create Quiz', 'Create Quiz',$view_level, 'edit.php?page=JibuPro/quiz_form.php&amp;action=new');
	add_submenu_page('quiz-plug', 'Quiz Results', 'Quiz Results',$view_level, 'edit.php?page=JibuPro/quiz_form.php&amp;action=result');
	add_submenu_page('quiz-plug', __('Manage Quiz', 'JibuPro'), __('Manage Quiz', 'JibuPro'), $view_level, 'JibuPro/quiz.php');
	$code_pages = array('quiz_form.php','quiz_action.php', 'question_form.php', 'question.php');	foreach($code_pages as $code_page) {		$hookname = get_plugin_page_hookname("JibuPro/$code_page", '' );		$_registered_pages[$hookname] = true;
	}    $hookname = get_plugin_page_hookname( "JibuPro/quiz.php" , '' );
    $_registered_pages[$hookname] = true;
...
function JibuPro_options() {
	...
	require(ABSPATH. '/wp-content/plugins/JibuPro/options.php');
}function readmeQuiz(){
 	$image = home_url().'/wp-content/uploads/JibuPro/logo.png';
...
function JibuPro_shortcode( $attr ) {
	...
	if(is_numeric($quiz_id)) { // Basic validiation - more on the show_quiz.php file.
		ob_start();
		include(ABSPATH . 'wp-content/plugins/JibuPro/show_quiz.php');
...
function JibuPro_shortcode2() {
	include(ABSPATH . 'wp-content/plugins/JibuPro/user_result.php');
}
...
$upload = wp_upload_dir();
	$jibupro_dir = $upload['basedir'].'/JibuPro';
	$newImage = $jibupro_dir.'/logo.png';
	if(!is_dir($jibupro_dir)){ mkdir($jibupro_dir); }
    if(!is_file($newImage)){
		copy(ABSPATH.'wp-content/plugins/JibuPro/logo.png',$newImage);
	}	 
	// Initial options.
